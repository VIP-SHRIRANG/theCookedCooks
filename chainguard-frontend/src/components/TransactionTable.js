...
function TransactionTable({ privacyMode }) {
  ...
  const getRiskColor = (riskScore) => {
    if (riskScore >= 80) return 'error';      // Critical - Red
    if (riskScore >= 65) return 'warning';    // High Risk - Orange  
    if (riskScore >= 30) return 'info';       // Medium - Blue
    if (riskScore >= 30) return 'secondary';  // Medium - Purple
    return 'success';                         // Normal - Green
  };

  const getLabelColor = (label) => {
    if (label === 'Fraudulent') return 'error';
                    ...
                    >
                      ...
                      <TableCell align="right">
                        <Box>
                          ...
                          >
                            {tx.Value ? (parseFloat(tx.Value) / 1e18).toFixed(6) : '0.000000'}
                          </Typography>
                          <Typography 
                            variant="caption" 
                            color="text.secondary"
